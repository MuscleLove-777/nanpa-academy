...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('nanpa-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('nanpa-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('nanpa-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('nanpa-progress', JSON.stringify(this.progress));
        localStorage.setItem('nanpa-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        let html = '<div class="fade-in">' +
            '<div class="dashboard-hero"><h2>ナンパアカデミー へようこそ</h2>' +
            '<p>声かけから始まるコミュニケーション戦略を体系的に学びましょう</p></div>' +
        ...
        this.levels.forEach(level => {
            ...
            html += '<div class="level-card level-' + level.id + '" onclick="App.showModule(' + levelMods[0].id + ')">' +
                ...
                '<div class="level-progress"><div class="level-progress-bar">' +
    ...
    showModule(moduleId) {
        ...
        if (prevMod) {
            html += '<button class="btn btn-outline" onclick="App.showModule(' + prevMod.id + ')">← ' + prevMod.title + '</button>';
        }
        ...
        if (nextMod) {
            html += '<button class="btn btn-outline" onclick="App.showModule(' + nextMod.id + ')">' + nextMod.title + ' →</button>';
        }
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('nanpa-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('nanpa-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('nanpa-progress');
            localStorage.removeItem('nanpa-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
